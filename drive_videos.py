...
# --- Helper to parse explicit batch groups from MODELS string ---
def parse_models_groups(models_str: str):
    """
    Return a list of batches (each batch is a list of parsed model dicts) when the MODELS
    string uses '|' separators, e.g. "pytorch:candy | pytorch:mosaic,pytorch:udnie | torch7:the_scream.t7".
    If the string doesn't contain any '|', return an empty list to indicate 'no explicit batches'.
    """
    if not models_str or "|" not in models_str:
        return []
    groups = [g.strip() for g in models_str.split("|") if g.strip()]
    batches = []
    for g in groups:
        batches.append(parse_models_env(g))
    return batches
...
def resolve_model_for_slot(m, PYTORCH_DIR, TORCH_DIR, MAGENTA_DIR, MAGENTA_STYLES_DIR):
    ...
    if t == 'reconet':
        p = pathlib.Path(ident)
        if not p.is_absolute():
            # accept either exact filename or basename without suffix
            p = pathlib.Path(RECONET_DIR) / (ident if pathlib.Path(ident).suffix else f"{ident}.pth")
        return (str(p), 'reconet', '')
...
def assemble_montage(clips: list[pathlib.Path], output_path: pathlib.Path, segment_secs: float, fade_secs: float, intro_secs: float) -> None:
    ...
    - Robust to zero/near-zero durations: any clip with duration ≤ eps is dropped.
    """
    if not clips:
        raise SystemExit("assemble_montage: no clips to assemble")

    eps = 1e-6  # treat extremely small durations as zero to avoid ffmpeg graph errors
    # Build list of (clip_path, duration) and drop zero-length entries
    planned = []
    for i, c in enumerate(clips):
        dur = float(intro_secs) if i == 0 else float(segment_secs)
        if dur > eps:
            planned.append((c, dur))
        else:
            print(f"[montage] Skipping clip index {i} ({c.name}) due to non-positive duration={dur}")

    if not planned:
        raise SystemExit("assemble_montage: all planned durations were ≤ 0; nothing to assemble")

    # If only one effective clip remains, just trim/re-encode it to the requested duration/fps.
    if len(planned) == 1:
        c, dur = planned[0]
        fps_str = os.getenv("FPS", "24")
        filter_complex = f"[0:v]trim=duration={dur},setpts=PTS-STARTPTS,fps=fps={fps_str}[v0]"
        cmd = ["ffmpeg","-y","-i", str(c), "-filter_complex", filter_complex, "-map", "[v0]", "-c:v","libx264","-pix_fmt","yuv420p", str(output_path)]
        print(f"[montage] Assembling single clip → {output_path.name}")
        print(f"[montage] filter_complex=\n{filter_complex}")
        subprocess.run(cmd, check=True)
        return

    # Multiple clips: build inputs
    inputs = []
    for c, _ in planned:
        inputs += ["-i", str(c)]
    ...
    for i, (_, dur) in enumerate(planned):
        filter_parts.append(
            f"[{i}:v]trim=duration={dur},setpts=PTS-STARTPTS,fps=fps={fps_str}[v{i}]"
        )
        labels.append(f"[v{i}]")

    # Chain xfades with cumulative offsets based on each clip's duration
    out_label = "[v0]"
    x_idx = 0
    # First offset is previous-duration - fade_secs
    first_dur = planned[0][1]
    cumulative_offset = max(0.0, first_dur - float(fade_secs))
    ...
    for i in range(1, len(planned)):
        ...
        out_label = out
        # Advance by the *trimmed* duration of the current (second) input minus fade
        # so each subsequent offset aligns end-to-start with fade overlap.
        this_dur = planned[i][1]
        cumulative_offset += max(0.0, this_dur - float(fade_secs))

    filter_complex = ";".join(filter_parts)
    cmd = ["ffmpeg","-y", *inputs, "-filter_complex", filter_complex, "-map", out_label, "-c:v","libx264","-pix_fmt","yuv420p", str(output_path)]
    print(f"[montage] Assembling {len(planned)} clips → {output_path.name}")
    print(f"[montage] filter_complex=\n{filter_complex}")
    subprocess.run(cmd, check=True)
...
RECONET_DIR = os.getenv("RECONET_DIR", "/app/models/reconet")
...
for vid in vids:
    ...
    if models_env:
        ...
        if _truthy(os.getenv("MONTAGE", "0")):
            # Optional explicit batch grouping via '|' separators in MODELS.
            # Example: "pytorch:candy | pytorch:mosaic,pytorch:udnie | torch7:the_scream.t7"
            explicit_batches = parse_models_groups(models_env)
            ...
            if explicit_batches:
                # One output clip per explicit batch group
                N = len(explicit_batches)
            else:
                if batch_size == 1:
            ...
            clips = []
            if explicit_batches:
                # Respect user-provided group sizes per batch (1..4 models each)
                for i, group in enumerate(explicit_batches, start=1):
                    if not group:
                        continue
                    start = starts[i-1]
                    if len(group) == 1:
                        clip = render_single_model_clip(
                            vid, group[0], i, base_env,
                            max_frames=seg_frames, start_secs=start, dur_secs=clip_len
                        )
                    else:
                        if len(group) > 4:
                            raise SystemExit(f"Explicit batch {i} requests {len(group)} models; maximum per batch is 4.")
                        clip = render_batch_clip(
                            vid, group, i, base_env,
                            max_frames=seg_frames, start_secs=start, dur_secs=clip_len
                        )
                    clips.append(clip)
            else:
            ...
            # (Optional) Log explicit batch groups before assembling the montage
            if explicit_batches:
                for bi, grp in enumerate(explicit_batches, start=1):
                    print(f"[montage] Batch {bi}: " + ", ".join(f"{m.get('type')}:{m.get('id')}" for m in grp))

#!/usr/bin/env python3
"""
Optical Flow Slideshow - Magenta & Resolution Mix Only
"""
...
def create_filtered_slideshow(image_dir, output_path, pattern,
                              hold_frames=0, interp_frames=120,
                              fps=24, target_size=(1280, 720)):
    """Create slideshow with only images matching the pattern."""
    ...
    image_files = [f for f in all_images if re.search(pattern, os.path.basename(f))]
    image_files.sort()

    if len(image_files) < 2:
        print(f"Need at least 2 images matching pattern '{pattern}'")
        return

    print(f"Processing {len(image_files)} images matching '{pattern}'...")
...
if __name__ == '__main__':
    ...
    output_path = '/Users/trentmahaffey/Dev/NeuralStyleTransferV1/output/magenta_resmix_optflow.mp4'

    # Match magenta_ or resmix_ prefixes
    pattern = r'^(magenta_|resmix_)'

    create_filtered_slideshow(
        image_dir=image_dir,
